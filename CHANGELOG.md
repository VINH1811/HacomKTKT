...
- **Chức năng mới — kiểm tra nhất quán đơn giá NỘI BỘ trong một hồ sơ chào giá** (`core/internal_consistency.py`). Cùng một hạng mục nhưng nhà thầu chào nhiều đơn giá khác nhau ở các dòng/sheet khác nhau là dấu hiệu điển hình của việc sao chép giữa các sheet rồi sửa sót. Trước đây không phép kiểm nào bắt được: mọi so sánh khác đều cần ít nhất hai tài liệu (hồ sơ với phụ lục, hoặc nhà thầu này với nhà thầu kia); riêng trường hợp **một nhà thầu duy nhất** thì đây là phép kiểm tra giá duy nhất còn dùng được.
  - Gom nhóm chặt để tránh báo oan: phải khớp **cả mã hiệu lẫn tên** (hồ sơ cơ điện thường dùng một mã định mức cho nhiều quy cách — mã tôn `Z08-1.15` dùng cho mọi cỡ ống gió); tên rút gọn kiểu `KT: 1500x500` phải cùng mục cha; bỏ qua đơn vị trọn gói (lô, gói, hệ thống) vì mỗi dòng là một phạm vi riêng; bỏ qua tiêu đề mục viết hoa toàn bộ và ghi chú bị đọc nhầm vào cột mã (`CĐT cấp`). Bỏ qua chênh lệch dưới 0,5% (làm tròn).
  - Cảnh báo ghi rõ từng dòng và từng mức giá để chuyên viên mở file kiểm lại đúng vị trí; phân biệt phạm vi **cùng sheet** (gần như chắc chắn sai sót) với **khác sheet** (có thể hợp lý do khác vị trí lắp đặt).
  - Sửa kèm: cờ chất lượng dữ liệu trước đây **bị mất trong chế độ so sánh ngang nhiều nhà thầu** vì luồng đó không đi qua `build_bidder_rows`; nay được gắn trong `build_peer_consensus` nên mọi cảnh báo (lỗi công thức, lệch giá nội bộ...) đều tới được kết quả.
  - Kiểm chứng trên 4 hồ sơ thật: phát hiện 16–75 nhóm lệch giá mỗi hồ sơ (ví dụ cùng mã và cùng tên nhưng chênh 41–93%); chi phí chỉ 45 ms trên hồ sơ 4.796 hạng mục (1,8% thời gian đọc).

- **Ba lỗi từ phản hồi thực tế khi chấm gói PCCC/Thông gió:**
  - *Ghép nhầm chủng loại.* Ống thép chữa cháy bị ghép với ống luồn dây điện, đầu báo khói với đầu báo nhiệt, cáp Cu/XLPE/PVC với cáp Cu/PVC — làm mô tả và thương hiệu hiển thị sai. Nguyên nhân: từ phân biệt bị chìm trong phần tên chung nên điểm giống chữ vẫn rất cao (0,70–0,95, trong khi ngưỡng loại bỏ chỉ 0,58). Bổ sung bộ phát hiện **xung khắc chủng loại** theo 5 nhóm thuộc tính loại trừ nhau (vật liệu ống, loại đầu báo, chất chữa cháy, hệ thống phục vụ, cách điện cáp): khi cả hai mô tả cùng nêu thuộc tính của một nhóm nhưng nêu khác nhau thì chặn ghép mờ, để thành thiếu/phát sinh cho người rà soát thay vì ghép sai. Chỉ áp cho các tầng ghép mờ (fuzzy, gần dòng, ngữ nghĩa) — tầng khớp mã hiệu/cấu trúc không đổi.
  - *Cột chi phí quản lý bị bỏ trống* dù nhà thầu có chào, do hồ sơ viết "Quản lý", "CP quản lý", "CPQL", "QL", "Chi phí chung" thay vì "CF quản lý"; riêng "CPQL"/"QL"/"Chi phí chung" còn bị nhận **nhầm thành đơn giá tổng hợp**. Nay nhận đủ 7 biến thể và không nuốt mất cột ĐG tổng hợp; bổ sung tương tự cho lợi nhuận ("LN", "lãi định mức").
  - *Ô "Mô tả/Quy cách" đầy ghi chú lạc đề* (thiếu đơn giá, tổng hợp giá...). Trước đây mọi sai lệch không có ô riêng đều bị dồn vào ô này. Nay chỉ những sai lệch nói về chính hàng hóa (tên hạng mục, thông số, quy cách) mới vào ô Mô tả; cảnh báo về trạng thái dòng và chất lượng dữ liệu được bỏ khỏi bảng tổng hợp vì đã hiển thị đầy đủ ở sheet Bất thường và AI_KIEM_TRA. Cột "KL mời thầu trong file" nay tô đúng ô khối lượng thay vì ô Mô tả.
...
- **RFI Tracker** (`core/rfi_tracker.py`): đọc file "Nội dung làm rõ HSCG" của CĐT (cột Ý kiến CĐT = yêu cầu), ghép từng yêu cầu với dòng tương ứng trong file phản hồi của nhà thầu (fuzzy, chịu chèn/sửa dòng), chấm ĐÃ TRẢ LỜI / CHƯA TRẢ LỜI / KHÔNG THẤY TRONG PHẢN HỒI. Chạy thật phát hiện 1 yêu cầu Linh Anh (PL 2 mục 2.1) chưa trả lời.
- **Đánh giá tính đầy đủ hồ sơ** (`core/dossier_check.py`): quét cây thư mục hồ sơ mỗi nhà thầu, nhận diện 12 đầu mục tài liệu (đơn chào giá, BOQ, BCTC 3 năm, hợp đồng tương tự, nhân sự, BPTC, ATLĐ...) không phân biệt hoa thường/dấu tiếng Việt → ma trận ĐẠT/THIẾU × nhà thầu + sheet bằng chứng.
- **Sheet "Tổng hợp 03.1" công thức sống**: file tổng hợp nhiều nhà thầu nay mở đầu bằng sheet rollup kiểu Mẫu số 03.1 (mỗi hạng mục hệ thống một dòng, mỗi nhà thầu hai cột Theo KLMT / Nhà thầu chào, dòng A/B/C) với TOÀN BỘ số liệu là công thức `=SUM('sheet'!...)` trỏ về dải mục A/B của từng sheet chi tiết — sửa số trong sheet chi tiết là tổng tự cập nhật, đúng file CĐT vẫn làm tay.
