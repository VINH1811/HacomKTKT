...
- File nhà thầu đã đánh dấu: sheet sắp xếp bám ĐÚNG cấu trúc **A / B / C** của file chào giá — hạng mục phát sinh (không có trong PL01) đang nằm lẫn trong **phần A** được DỜI xuống **phần B (Phát sinh ngoài KLMT)**, giữ nguyên các mục B sẵn có; nếu file chưa có phần B thì tự tạo. Format y hệt bản gốc (sao chép nguyên style/đầu mục). Ba dòng tổng A/B/C được viết lại theo dải dòng mới (A, B là `SUBTOTAL`; C = A + B) nên số tiền luôn đúng; công thức thành tiền trong-dòng vẫn "sống". Sheet sắp xếp mang TÊN GỐC; bản gốc đổi thành `<tên> — gốc` (giữ nguyên toàn bộ) và mọi tham chiếu chéo (`Tổng hợp`, hyperlink `AI_KIEM_TRA`, chart) được viết lại trỏ đúng.
