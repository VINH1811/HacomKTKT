...
def _annotation_payload(bidder: WorkbookData, rows: list[ComparedItem]) -> tuple[WorkbookData, list[ComparedItem]]:
    ...
    đọc items/raw/technical_specs (cấu trúc A/B/C của sheet sắp xếp được đọc
    trực tiếp từ file gốc). Loại chúng khỏi payload giảm ~10 lần chi phí truyền
    dữ liệu giữa tiến trình mà file kết quả giữ nguyên (đã có test đối chứng
    process-vs-tuần-tự bảo vệ).
    """
...
def compare_appendices_with_bidders(
    ...
    *,
    ...
    if pl1:
        ...
        result.warnings.extend(scope_mismatch_warnings("Phụ lục 01", rows))
